...
func ExampleBlazegraph_EmptyRequest_HttpGet_StatusOK() {
	response, _ := http.Get(blazegraph.DefaultSparqlEndpoint)
...
func ExampleBlazegraph_EmptyRequest_HttpClientGet_StatusOK() {
	client := &http.Client{}
	response, _ := client.Get(blazegraph.DefaultSparqlEndpoint)
	fmt.Println(response.Status)
	// Output:
	// 200 OK
}
...
func ExampleBlazegraph_EmptyRequest_HttpClientDoGet_StatusOK() {
	client := &http.Client{}
	request, _ := http.NewRequest("GET", blazegraph.DefaultSparqlEndpoint, nil)
...
func ExampleBlazegraph_PostSparqlRequest_SelectAllTriples() {
	client := &http.Client{}
	request, _ := http.NewRequest("POST", blazegraph.DefaultSparqlEndpoint, strings.NewReader(`
		SELECT ?s ?p ?o
		WHERE
		{}
	`))
...
// 	client := &http.Client{}
// 	request, _ := http.NewRequest("DELETE", DefaultSparqlEndpoint+"/kb", nil)
